...
class StationSerializer(serializers.ModelSerializer):
    class Meta:
        model = Station
        fields = ["id", "slug", "name", "status", "image", "longitude", "latitude"]

    def to_representation(self, instance):
        total_slots = len(Slot.objects.filter(station_id=instance.id))
        total_bikes = len(Slot.objects.filter(station_id=instance.id, status="used"))
        return {
            "id": instance.id,
            "slug": instance.slug,
            "name": instance.name,
...
class SlotSerializer(serializers.ModelSerializer):
    ...
    def create(context):
        ...
        slot = Slot.objects.create(station_id=station_id, bike_id=None, status="unused")
        slot.save()
        return slot
    ...
    def create_slot_filler(context):
        ...
        if status == "used" and len(bikes) > 0:
            ...
            slot = Slot.objects.create(
                station_id=station_id, bike_id=bike.id, status="used"
            )
            bike.status = "unused"
            bike.save()
        else:
            slot = Slot.objects.create(
                station_id=station_id, bike_id=None, status="unused"
            )
        slot.save()
        return slot
