...
class IncidentSerializer(serializers.ModelSerializer):
    ...
    def create(context):
        ...
        incident = Incident.objects.create(
            title=title, body=body, user_id=user.id, slot_id=slot.id
        )
...
class NotificationSerializer(serializers.ModelSerializer):
    class Meta:
        model = Notification
        fields = ["id", "body", "seen", "created_at", "modified_at"]
    ...
    def seeNotification(context):
        ...
        notification = Notification.objects.get(
            pk=notification_id, user_id=user.id, seen=False
        )
        if notification is None:
            raise serializers.ValidationError("notification is not find")
