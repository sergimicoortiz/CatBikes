    ...
    Mutation: {
        ...
        deleteStation: async (parent, args, context) => {
            try {
                ...
                if (!station) throw new GraphQLError('Station not found', { extensions: { code: ApolloServerErrorCode.INTERNAL_SERVER_ERROR } });
        ...
        updateStation: async (parent, args, context) => {
            try {
                ...
                if (!name && !status && !image && !latitude && !longitude) throw new GraphQLError("No data to update", { extensions: { code: ApolloServerErrorCode.BAD_USER_INPUT } });
