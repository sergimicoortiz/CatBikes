...
const InfiniteScrollStations = () => {
    ...
    useEffect(() => {
        const limit = cont * 2;
        const items_tmp = [...stations].splice(0, limit).map((item) => {
            return (
                <div
                    className="card"
                    key={item.slug}
                ...
                >
                    <div className="content">
                        ...
                        <span className="copy">
                            Bikes for Rent: {item.total_bikes}
                        </span>
                        <button
                            className="btn"
                            onClick={() => {
        ...
        setItems(items_tmp);
        if (items_tmp.length === stations.length && items_tmp.length > 0)
            setHasMore(false);
    }, [cont]);
    ...
    return (
        ...
        >
            <div className="stationsClientCard">
                <main className="page-content-">{items}</main>
            </div>
        </InfiniteScroll>
    );
};
